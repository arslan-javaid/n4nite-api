...
//********************************************************************************************************
    ...
    MATCH (e:entity:businessObject)-[r1:HAS_STATE{current:true}]->(s1:state)
        WHERE id(e) = $id
    SET r1.current = false, r1.to = timestamp()
    CREATE (e)-[r2:HAS_STATE {from:timestamp(), to:9223372036854775807, current:true, modifiedBy:"mos"}]->(s2:state:businessObjectState)
